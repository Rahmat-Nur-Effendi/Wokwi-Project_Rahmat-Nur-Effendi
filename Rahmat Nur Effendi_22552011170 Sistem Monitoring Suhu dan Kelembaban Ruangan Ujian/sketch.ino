...
#define BUZZER 12  // Buzzer di D12
...
unsigned long myChannelNumber = 3010442;
const char * myWriteAPIKey = "FY4TF7JK9XMTFCU7";
...
void loop() {
  ...
  display.clearDisplay();
  // display.drawBitmap(0, 5, suhu_icon, 55, 55, WHITE);
  display.setCursor(0, 25);        // Baris pertama
  display.print("Suhu     ");      // teks label

  display.setCursor(0, 40);        // Baris kedua
  display.print("Hum ");
  ...
  if (currentMillis - previousMillis >= ts_update_interval) { 
   ...
   // 🛎️ Logika Alarm Buzzer
  if (temp < 20.0 || temp > 30.0) {
  tone(BUZZER, 1000); // 1000 Hz
  delay(1000);        // bunyikan selama 1 detik
